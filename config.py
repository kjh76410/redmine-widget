...
# 토스트 너비(물리 px). 300일 때는 일감 제목이 18자쯤에서 잘렸는데, 실제 제목이 그보다
# 긴 경우가 많아서 무슨 일감인지 알아보기 어려웠다(toast.css #line2가 한 줄 말줄임이다).
# 380이면 좌우 패딩 44px을 빼고 25자쯤 들어간다 - 창은 아이콘 오른쪽(x=아이콘+68)에서
# 시작하므로 더 넓혀도 화면에 걸리진 않지만, 알림이 여러 개 쌓이면 화면을 그만큼 가린다.
TOAST_W      = 380        # 새 이슈 알림 토스트 너비(px)
